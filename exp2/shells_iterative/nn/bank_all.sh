...
for use_bar_feature in ${use_bar_feature_list[@]}; do
    for obj_lam in ${obj_lam_list[@]}; do
        for seed in ${seed_list[@]}; do
            tmp_yaml="${config_dir}/tmp_iterative_${base_name}_${method}_${seed}_${use_bar_feature}.yaml"
            ...
            echo "nn_beta: ${nn_beta}" >> ${tmp_yaml}
            
            echo "is_iterative: True" >> ${tmp_yaml}
            echo "obj_lam: ${obj_lam}" >> ${tmp_yaml}
            echo "start_round: ${start_round}" >> ${tmp_yaml}
            echo "iterative_round: ${iterative_round}" >> ${tmp_yaml}
            echo "pred_loss_func: ${pred_loss_func}" >> ${tmp_yaml}

            python iterative_learning.py --config_file ${tmp_yaml}
            rm ${tmp_yaml}
        done
    done
done