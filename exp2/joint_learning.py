...
def run(args, logger):
    ...
    disamb_soft_df = comp_data_df.copy(deep=True) 
    disamb_hard_df = comp_data_df.copy(deep=True) 
    ...
    # prepare log
    pred_model_score_dict = {'soft': {}, 'hard':{}} # Save the model's prediction score when using data where the CFs' exact values is estimated only from the training data.
    ...
    # define feature estimation models
    feat_est_models = {}
    for comp_col in comp_feature_list:
        if args.arch == 'mlp':
    ...
    if args.arch == 'mlp':
        assert args.hd is not None
        label_pred_model_soft = mlp_model(input_dim=disamb_soft_df.drop(['target'], axis=1).shape[1], hidden_dim=args.hd, output_dim=num_classes_dict['target'])
    elif args.arch == 'mlp4':
        assert len(args.hds) == 4
        label_pred_model_soft = mlp_4layers(input_dim=disamb_soft_df.drop(['target'], axis=1).shape[1], hidden_dims=args.hds, output_dim=num_classes_dict['target'])
    ...
    # joint learning 
    label_pred_model_soft, feat_est_models, save_table_dict, train_score_dicts, test_score_dicts = \
                joint_learning(feat_est_models=feat_est_models, label_pred_model=label_pred_model_soft,
                                ...
                                comp_train_df=nn_comp_train_df, comp_test_df=nn_comp_test_df, ord_train_df=nn_ord_train_df, ord_test_df=nn_ord_test_df,  
    ...
    pred_model_scores_soft = evaluating(test_df=disamb_soft_df.loc[test_index, :], model=label_pred_model_soft, target_col='target', task=args.task, n_classes=n_classes, is_torch=True)

    # save
    pred_model_score_dict['soft'] = copy.deepcopy(pred_model_scores_soft)
    ...
    log_dict['save_tables'] = save_table_dict
    log_dict['train_score_dicts'] = train_score_dicts
    log_dict['test_score_dicts'] = test_score_dicts

    log_dict['est_model_dicts'] = feat_est_models
    log_dict['pred_model'] = {'soft': label_pred_model_soft}
